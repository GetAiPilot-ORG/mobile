...
import { WebviewAuthCodeService, WebviewTool } from '../services/webview-auth-code.service.js';
import { JWTPayload } from '../types/index.js';

const targetToolSchema = z.enum(['landing-builder', 'bio-builder', 'flow-builder', 'workflow-builder']);
...
export async function webviewRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/webview/session-token', { preHandler: [authenticateToken] }, async (request, reply) => {
    ...
    if (body.targetTool === 'bio-builder' && body.templateId) {
      const ownsTemplate = await HubAdapter.userOwnsBioTemplate(user.user_id, body.templateId);
      if (!ownsTemplate) {
    ...
    const targetUrl = new URL(`/tools/${body.targetTool}`, env.WEB_APP_URL);
  ...
  fastify.post('/webview/session-token/exchange', async (request, reply) => {
    ...
    if (!session) {
      return reply.status(401).send({
        statusCode: 401,
        error: 'Unauthorized',
        message: 'This builder link is invalid, expired, or has already been used.',
      });
    }
