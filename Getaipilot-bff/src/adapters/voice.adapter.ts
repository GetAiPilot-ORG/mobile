...
export class VoiceAdapter {
  ...
  private static async requestUpstream<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        err.code =
          response.status === 404
            ? "VOICE_UPSTREAM_ROUTE_NOT_FOUND"
      ...
      if (err.name === "AbortError") {
        const timeoutErr: any = new Error(
          "VoicePilot upstream request timed out",
        );
  ...
  public static async getOverview(
    ...
    const userObj =
      typeof user === "string"
        ...
        : user;
    try {
      const res: any = await this.requestUpstream("/api/v1/overview", {
        user: userObj,
      });
      ...
      console.warn(
        "[VOICE ADAPTER] Live upstream overview proxy bypassed, resolving via Voice Supabase & Vomyra",
      );
    }
    ...
    return {
      ...
      recentCalls: calls.slice(0, 5).map((c: any) => ({
        id: c.id,
        assistantName:
          c.assistant || c.assistantName || "Sales Representative Bot",
  ...
  public static async getCalls(
    ...
    try {
      ...
      console.warn(
        "[VOICE ADAPTER] Live upstream calls proxy bypassed, fetching from Vomyra",
      );
    }
    ...
    try {
      const vRes = await fetch(
        `https://api.vomyra.com/v1/calls?limit=${filters?.limit || 50}`,
        {
  ...
  public static async getCallDetails(user: JWTPayload, callId: string) {
    const res: any = await this.requestUpstream(`/api/v1/calls/${callId}`, {
      user,
    });
    return res.data || res;
  }
  ...
  public static async getCallTranscript(user: JWTPayload, callId: string) {
    const res: any = await this.requestUpstream(
  ...
  public static async getAgents(user: JWTPayload) {
    try {
      ...
      if (
        res &&
        (res.assistants || Array.isArray(res.data) || Array.isArray(res))
      ) {
        return res.assistants || res.data || res;
      }
    } catch (e) {
      console.warn(
        "[VOICE ADAPTER] Live upstream assistants proxy bypassed, resolving via Voice Supabase",
      );
    }
  ...
  public static async generatePrompt(
    ...
    const res: any = await this.requestUpstream(
      "/api/v1/assistants/generate-prompt",
      {
        method: "POST",
  ...
  public static async getCampaigns(user: JWTPayload) {
    ...
    if (error) {
      console.error("[VOICE CAMPAIGNS]", error);
      throw error;
    }
  ...
  public static async getPhoneNumbers(user: JWTPayload) {
    try {
      const res: any = await this.requestUpstream("/api/v1/phone-numbers/my", {
        user,
      });
  ...
  public static async getAvailableNumbers(user: JWTPayload) {
    try {
      ...
      if (res.available_numbers || res.data)
  ...
  public static async getContactDetails(user: JWTPayload, contactId: string) {
    ...
    const matched =
      contacts.find((c: any) => c.id === contactId) || contacts[0];
    return {
      ...matched,
      campaign_history: [
        {
      ...
      call_history: [
        {
          id: "call_1",
          time: "Sep 21, 2026 2:30 PM",
          duration: "1m 24s",
          status: "completed",
          assistant: "Sales Representative Bot",
          recording_url: "https://api.vomyra.com/recordings/sample.mp3",
        },
        {
          id: "call_2",
          time: "Sep 18, 2026 11:00 AM",
          duration: "0m 30s",
          status: "completed",
          assistant: "Sales Representative Bot",
        },
  ...
  public static async getUsage(user: JWTPayload) {
    try {
      const res: any = await this.requestUpstream("/api/v1/payments/usage", {
        user,
      });
      return res.data || res;
    } catch (e) {}
      ...
      creditLedger: [
        {
          id: "cld_1",
          type: "usage",
          description: "Outbound Call campaign (34 contacts)",
          credits: -51,
          date: "Sep 23, 2026, 12:40 PM",
        },
        {
          id: "cld_2",
          type: "usage",
          description: "Outbound Test Call (00:01:24)",
          credits: -2,
          date: "Sep 21, 2026, 02:30 PM",
        },
        {
          id: "cld_3",
          type: "topup",
          description: "Top-up pack credited",
          credits: +500,
          date: "Aug 20, 2026, 02:45 PM",
        },
      ],
