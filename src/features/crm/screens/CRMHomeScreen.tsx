...
export const CRMHomeScreen: React.FC<CRMHomeScreenProps> = ({
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: isDark ? '#0F1015' : '#F8FAFC' }]} edges={['top']}>
        ...
        <View style={styles.headerRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.headerSubtitle, { color: isDark ? '#9CA3AF' : '#64748B' }]} numberOfLines={1}>
          ...
          <View style={styles.quickActionRow}>
            ...
            <Pressable
              ...
              hitSlop={6}
            >
              <Ionicons name="checkbox-outline" size={14} color={isDark ? '#D1D5DB' : '#334155'} />
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statsRow}>
                <CrmStatCard
                  ...
                  sub={stats?.overdueTasks ? `${stats.overdueTasks} overdue` : 'On track'}
                  icon="checkbox-outline"
                  gradientColors={isDark ? ['#291807', '#140E04'] : ['#D97706', '#B45309']}
            ...
            <View style={styles.sectionBlock}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionTitleRow}>
                  <Ionicons name="git-commit-outline" size={16} color="#3B82F6" />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.stageScroll} contentContainerStyle={{ gap: 8 }}>
                {pipelineSummary.map((p) => (
                  <Pressable
                    key={p.stage}
                    style={[styles.stagePill, isDark ? styles.stagePillDark : styles.stagePillLight]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
                      onNavigateTab?.('pipeline');
                    }}
                  >
                    ...
                    <View style={[styles.stageCountBadge, { backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)' }]}>
            ...
            <View style={styles.sectionBlock}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionTitleRow}>
                  ...
                  <Text style={[styles.sectionTitle, { color: isDark ? '#FFFFFF' : '#0F172A' }]}>Today's Priority Follow-ups</Text>
                  {upcomingTasks.length > 0 && (
                    <View style={styles.badgeCount}>
                      <Text style={styles.badgeCountText}>{upcomingTasks.length}</Text>
                    </View>
                  )}
                </View>
              ...
              {upcomingTasks.length === 0 ? (
                <View style={[styles.emptyCard, isDark ? styles.cardDark : styles.cardLight]}>
                  ...
                  <Text style={[styles.emptyTitle, { color: isDark ? '#FFFFFF' : '#0F172A' }]}>All caught up for today!</Text>
                  <Text style={[styles.emptySubtitle, { color: isDark ? '#9CA3AF' : '#64748B' }]}>
                    No overdue tasks or urgent customer follow-ups pending.
                  </Text>
                  <Pressable
                    style={[styles.emptyBtn, isDark ? styles.emptyBtnDark : styles.emptyBtnLight]}
                    onPress={() => setShowAddTask(true)}
                  >
                    <Text style={styles.emptyBtnText}>+ Schedule Follow-up</Text>
                  </Pressable>
                </View>
              ) : (
                <ScrollView
                ...
                >
                  {upcomingTasks.map((task) => {
                    ...
                    return (
                      <View
                        key={task.id}
                        style={[styles.followupCard, isDark ? styles.cardDark : styles.cardLight]}
                      >
                        <View style={styles.followupTopRow}>
                          ...
                          >
                            <Ionicons
                              ...
                              size={22}
                              color={isDone ? '#10B981' : isDark ? '#6B7280' : '#94A3B8'}
                            />
                          </Pressable>
                        </View>
                        ...
                        {task.contact ? (
                          <View style={styles.followupContactRow}>
                            <Ionicons name="person-circle-outline" size={14} color={isDark ? '#9CA3AF' : '#64748B'} />
                            <Text style={[styles.followupContactName, { color: isDark ? '#9CA3AF' : '#64748B' }]} numberOfLines={1}>
                        ...
                        <View style={styles.followupFooter}>
                          ...
                          >
                            <Text
                              style={[
                                ...
                                { color: task.priority === 'urgent' ? '#EF4444' : isDark ? '#9CA3AF' : '#64748B' },
                              ]}
                            >
                          ...
                          <Pressable
                            ...
                            onPress={() => onNavigateTab?.('tasks')}
                          >
                            <Text style={styles.followupActionText}>View Details →</Text>
                          </Pressable>
                        </View>
                      </View>
                    );
            ...
            <View style={styles.sectionBlock}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionTitleRow}>
                  ...
                  <Text style={[styles.sectionTitle, { color: isDark ? '#FFFFFF' : '#0F172A' }]}>Recent Active Leads</Text>
                </View>
                <Pressable onPress={() => onNavigateTab?.('leads')}>
                  <Text style={styles.sectionLink}>View All ({stats?.totalLeads ?? recentLeads.length})</Text>
                </Pressable>
              </View>
              ...
              {recentLeads.length === 0 ? (
                <View style={[styles.emptyCard, isDark ? styles.cardDark : styles.cardLight]}>
                  ...
                  <Text style={[styles.emptySubtitle, { color: isDark ? '#9CA3AF' : '#64748B' }]}>
                    Add your first prospect or link WhatsApp contacts to build your sales pipeline.
                  </Text>
                  <Pressable
                    style={[styles.emptyBtn, isDark ? styles.emptyBtnDark : styles.emptyBtnLight]}
            ...
            <View style={styles.sectionBlock}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionTitleRow}>
                  ...
                  <Text style={[styles.sectionTitle, { color: isDark ? '#FFFFFF' : '#0F172A' }]}>Live Touchpoints & History</Text>
                </View>
              ...
              {recentActivities.length === 0 ? (
                <View style={[styles.emptyCard, isDark ? styles.cardDark : styles.cardLight]}>
                  ...
                  <Text style={[styles.emptyTitle, { color: isDark ? '#FFFFFF' : '#0F172A' }]}>No touchpoints logged</Text>
                  <Text style={[styles.emptySubtitle, { color: isDark ? '#9CA3AF' : '#64748B' }]}>
      ...
      {/* Floating Action Speed Dial (Thumb-Zone Ergonomics) */}
      {fabExpanded && (
